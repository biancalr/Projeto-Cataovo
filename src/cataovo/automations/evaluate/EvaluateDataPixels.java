 ...
 * @author bianc
 */
public class EvaluateDataPixels extends BasicEvaluate {
    ...
    protected float[] execute(final String regionsInFrame, final String pointsInFrame) throws DirectoryNotValidException {
        ...
        if (eggsString.size() >= 1) {
            ...
            List<List<Point>> foundEggs = new ArrayList<>();
            List<List<Point>> remanescent = new ArrayList<>();
            final List<Region> regioesEncontradas = new ArrayList<>();
...
//                            percentual++;
            ...
            // Calcular a área da cada um dos quadrantes dos ovos
            // deixar apenas os pontos contidos nas marcações manuais
            if (!foundEggs.isEmpty() && !pontosEncontradosAux.isEmpty() && !regioesEncontradas.isEmpty()) {
                ...
                totalEggsInPixels = 0;
                totalEggsInPixels = regioesEncontradas.stream().map(Region::getArea).reduce(totalEggsInPixels, Integer::sum);
